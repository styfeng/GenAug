...
def read_prompts_from_file(input_file_name):
    all_lines = open(input_file_name).readlines()
...
def main():
    ...
    if not args.multi_prompt:
        ...
        output_sequences = model.generate(
            ...
            max_length=min(args.length + len(encoded_prompt[0]),model.config.max_position_embeddings),
        ...
        for i,prompt_text in enumerate(prompt_texts):
            ...
            effective_max_length = min(args.length + len(encoded_prompt[0]),model.config.max_position_embeddings) #args.length + len(encoded_prompt[0])
            #print("Effective max length: ", effective_max_length)
            ...
            if args.num_return_sequences <= 20:
                ...
                for seq_frag in seq_frags:
                    output_sequences_frag = model.generate(
                        input_ids=encoded_prompt,
                        max_length=effective_max_length,
                        temperature=args.temperature,
                        top_k=args.k,
            ...
            for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
                ...
                #print(cleaned_text)
                #cleaned_text = cleaned_text[: cleaned_text.find(args.stop_token)] if args.stop_token else cleaned_text
                #print(cleaned_text)
