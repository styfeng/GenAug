...
def evaluate_bert_score(prompts, continuations, scorer):
    all_results = []
...
overall_results['BERTSCORE_NEW'], all_results = evaluate_bert_score(prompts, continuations, scorer)

#write overall average results to file
pprint(overall_results)
out_filename = continuation_file + '_BERTSCORE_NEW'
print("Writing final BERTSCORE result to file: ", out_filename)
with open(out_filename, "w") as fout:
    pprint(overall_results, stream=fout)
print("Final BERTSCORE result written to file: ", out_filename)

#write individual results to file (for statistical significance purposes later)
out_filename_lst = continuation_file + '_BERTSCORE_NEW_LIST'
print("Writing individual BERTSCORE results to file")
